...
(function () {
  ...
  async function doLogin(event) {
    ...
    try {
      ...
      $('#password').value = '';
      // Recolhe os dados da sessão (última entrada, validade, envio de e-mail)
      state.session = await api('/api/admin/session').catch(() => ({
        authenticated: true, mustChangePassword: result.mustChangePassword,
      }));
      await startApp();
  ...
  function renderBrandView() {
    ...
    view.appendChild(sectionCard(
      ...
      grid(
        ...
        makeField({ path: 'brand.tagline', label: 'Slogan', placeholder: 'Travel Booking · mil destinos, uma viagem só sua' }),
      ),
      makeField({
        path: 'brand.siteUrl', label: 'Endereço do site', placeholder: 'https://www.1000viagens.pt',
        hint: 'Domínio final, com https:// e sem barra no fim. É usado nos endereços que o Google indexa (canonical, partilhas e mapa do site) — escreva-o antes de pôr o site online.',
      }),
    ));
